...
**Fecha de actualizacion:** 2026-08-02
...
Este repo tenia la copia `c00054567c09fe647bd79e91586a046e`, anterior a la
plantilla vigente, que **corregia el proceso pero no exportaba la locale al
entorno**: los procesos hijos (`quarto`, `system2`, el R que quarto levanta
para los chunks de un `.qmd`) arrancaban en `C` igual. La copia se sobrescribio;
la version anterior queda en la historia de git.
...
Cuando la exportacion al entorno **no** ocurre (porque `LC_ALL` viene fijado, o
porque `LANG` y `LC_CTYPE` ya traen valor), el mensaje lo declara y nombra la
causa, en vez de afirmar una cobertura que no existe.
...
- Prohibido envolver `Sys.setlocale()` en `try(..., silent = TRUE)` o en
  `suppressWarnings()`.
- Si el repo tiene workflows de CI que ejecutan R: `env: LANG: C.UTF-8` a nivel
  de job (el default de los runners es POSIX).
...
ejecutable, V3 comportamiento del propio proceso bajo `LANG=C`, V4 que la
locale llegue a los procesos hijos.
